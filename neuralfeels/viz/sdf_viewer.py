...
# View mesh SDFs, used in data/mesh_to_sdf_test.py. SDFViewer taken from iSDF: https://github.com/facebookresearch/iSDF
...
class SDFViewer(trimesh.viewer.SceneViewer):
    def __init__(
        ...
        if sdf_pc is not None:
            ...
            if len(self.zs) > 500:
                self.zs = np.linspace(sdf_pc[:, 2].min(), sdf_pc[:, 2].max(), 40)
            ...
            assert sdf_grid.ndim == 3, "SDF grid must have 3 dims"
        ...
        if self.colormap:
            if sdf_range is None:
                if self.sdf_format == "pc":
                    ...
                    sdf_range = [sdf_grid_pc[..., 3].min(), sdf_grid_pc[..., 3].max()]
                else:
        ...
        if open_window:
            ...
            super().__init__(scene, callback=callback, resolution=(1080, 720))
            if wireframe:
    ...
    def get_slice_pc(self):
        if self.sdf_format == "pc":
            ...
            sdf_slice_pc = np.repeat(full_slice[None, ..., :4], self.n_grids, axis=0)
            sdf_slice_pc[:, :, :, 3] = full_slice[..., -self.n_grids :].transpose(
                2, 0, 1
            )
            for i in range(1, self.n_grids):
                shift = i * self.offset
        ...
        if not self.colormap:
            ...
            col = np.full(sdf_slice_pc.shape, np.array([1.0, 0.0, 0.0, 1.0]))
            # col[sdf_slice_pc[:, 3] > 0] = np.array([0., 1., 0., 1.])
            col[sdf_slice_pc[:, 3] == 0] = np.array([0.0, 1.0, 0.0, 1.0])
            col[sdf_slice_pc[:, 3] > 0] = np.array([0.0, 0.0, 1.0, 1.0])

        else:
            col = self.colormap_fn.to_rgba(sdf_slice_pc[:, 3], alpha=1.0, bytes=False)
    ...
    def project_rays(self, scene):
        if self.z_ix < len(self.ray_lens):
            ...
            lines = torch.cat((self.ray_origins[:, None, :], ends[:, None, :]), dim=1)
    ...
    def save_level_sets(self, scene):
        if self.lim_ix < len(self.limits):
            ...
            with self.lock:
                self.scene.delete_geometry("pc")
                if len(pts) > 0:
                    col = self.colormap_fn.to_rgba(pts[:, 3], alpha=1.0, bytes=False)
        ...
        if self.pose_ix < len(self.poses):
            ...
            with self.lock:
                ...
                camera = trimesh.scene.Camera(
                    fov=self.scene.camera.fov, resolution=self.scene.camera.resolution
                )
                marker = draw.draw_camera(
                    camera,
                    self.poses[self.pose_ix],
...
def save_slice(fname, pc, col, colormap_fn):
    ...
    extent = [
        0,
        np.linalg.norm(pc[0, 0, :2] - pc[-1, 0, :2]),
        0,
        np.linalg.norm(pc[0, 0, :2] - pc[0, -1, :2]),
    ]
