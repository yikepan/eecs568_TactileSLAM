            ...
            for visual_xml in visuals_xml:
                ...
                if geometry_xml != None:
                    ...
                    if mesh_xml != None:
                        ...
                        if "scale" in mesh_xml.attrib:
                            scale = parseThreeNumber(mesh_xml.attrib["scale"])
                            link.setVisualMeshScale(scale)

                if visual_xml.find("material"):
                    color_xml = visual_xml.find("material").find("color")
                    if color_xml != None:
                        link.color = np.fromstring(
                            color_xml.attrib["rgba"], dtype=float, sep=" "
                        )[:3]
